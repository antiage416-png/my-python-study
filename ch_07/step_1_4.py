...
def table_to_dataframe(header: list, body: list) -> pd.DataFrame:
    ...
    df_raw = df_raw.dropna(how="any") # 하나의 열이라도 데이터가 없으면 행삭제
    df_raw = df_raw.iloc[:,:-1] # 마지막열 삭제
    for col in df_raw.columns:
        df_raw[col] = df_raw[col].apply(clean_white_space) # 열별로 공백문자 정제
    return df_raw

if __name__ == '__main__':
    parsed = json.loads(OUT_1_3.read_text(encoding='utf-8')) # JSON 파일 불러오기

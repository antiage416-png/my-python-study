from pathlib import Path
import streamlit as st
from step_1 import OUT_DIR
from step_3_3 import OUT_3_3, read_text_and_fill_area

st.title('v 만들면서 배우는 문자인식 웹앱') # 웹 앱 제목

uploaded = st.file_uploader('인식할 이미지를 선택하세요') # 파일 업로더 위젯
if uploaded is not None: # 파일 업로드 되면,
    tmp_path = OUT_DIR / f'{Path(__file__).stem}.tmp' # 임시파일경로
    tmp_path.write_bytes(uploaded.getvalue()) # 업로드한 이미지 저장

    col_left, col_right = st.columns(2)
    with col_left: # 첫 번째 열
        st.subheader('원본 이미지') # 부제목
        st.image(tmp_path.as_posix()) # 원본 이미지 출력
    with col_right: # 두번째 열
        st.subheader('문자인식 결과') # 부제목
        with st.spinner(text='문자인식을 하는 중입니다...'): # 진행상황 표시
            read_text_and_fill_area(tmp_path ) # 문자인식 및 바운딩 박스 채우기
        st.image(OUT_3_3.as_posix()) # 결과 이미지 출력

...
st.title('✌️만들면서 배우는 문자 인식 웹 앱') # 웹 앱 제목

uploaded = st.file_uploader('인식할 이미지를 선택하세요') # 파일 업로더 위쳇

if uploaded is not None: # 파일이 업로드 되면,
    tmp_path = OUT_2_3 / f'{Path(__file__)}.tmp' # 임시 파일 경로
    ...
    with col_right: # 두 번째 열
        ...
        with st.spinner(text='문자를 인식하는 중입니다...'): # 진행 상황 표시
            read_text_and_draw_line(tmp_path) # 문자인식 및 박스 그리기

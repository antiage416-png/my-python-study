...
from docx.shared import Cm # 크기 설정 클래스(센치미터)
...
def add_table(doc: DocumentObject, category: str, option: str):
    fetch_trends_by_filter(category, option) # 쇼핑 트렌드 수집
    ...
    for tr in table.rows: # 행 반복 처리
        for td in tr.cells: # 열 반복 처리
            if len(imgs_path) > 0: #처리할 경로가 남아있는 경우
                ...
                p_cell.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER # 단짝 정렬

    doc.add_paragraph('') # 빈단락추가

...
from docx.document import Document as DocumnetObject # Document 객체
...
def init_docx() -> DocumnetObject:
    ...
    date_string = dt.now().strftime(' (%Y.%m.%d)') # 현재날짜
    apply_font_style(p1.add_run(date_string), size_pt=15)
    return doc # Document 객체 전환
